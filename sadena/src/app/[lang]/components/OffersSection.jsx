...
export default function OffersSection({ lang, t, products = [], banners = [] }) {
  const dir = lang === 'ar' ? 'rtl' : 'ltr';
  const items = products?.slice(0, 8); // Showing up to 8 to match other sections, change to 4 if you prefer a single row
  ...
  return (
    <section 
      dir={dir} 
      // Using transparent background so the global organic pattern flows seamlessly
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            ...
            <div className="relative inline-flex items-center justify-center px-10 py-3 self-start">
              ...
              <h2 className="relative z-10 font-display text-xl sm:text-2xl font-bold text-black tracking-tight">
                {t?.offers?.heading || (lang === 'ar' ? 'العروض المميزة' : 'Special Offers')}
              </h2>
            </div>

            {/* "View Offers" Link */}
            <Link
              href={`/${lang}/products?category=offers`}
            ...
            {banners.map((banner, index) => (
              ...
              >
                ...
                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4">
                  ...
                  {/* Dynamic CTA Button */}
                  <Link
                    href={banner.link || `/${lang}/products?category=offers`}
